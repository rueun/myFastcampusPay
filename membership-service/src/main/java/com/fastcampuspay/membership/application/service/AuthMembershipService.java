...
public class AuthMembershipService implements AuthMembershipUseCase {
    ...
    public JwtToken refreshJwtTokenByRefreshToken(RefreshTokenCommand command) {
        ...
        if (!membershipJpaEntity.getRefreshToken().equals(command.getRefreshToken())) {return null;}

        // 고객의 refresh token 정보와, 요청받은 refresh token 정보가 일치하는지 확인 된 상태.
        if (!membershipJpaEntity.isValid()){return null;}

        String newJwtToken = authMembershipPort.generateJwtToken(new Membership.MembershipId(membershipIdString));
